...
function execServer(onStarted) {
    ...
    const child = proc.spawn("node", ["./tools/server/start.js", 3001, "tools/data/testdb.json"], {stdio: "inherit"});
...
var optsPath = argv.opts ? path.resolve(process.cwd(), argv.opts) : path.resolve(process.cwd(), "mocha.json");
...
var command = createMochaCommand(mochaOptions.opts);
